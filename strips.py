...
class StripStructure(object):
    def __init__(self, extent, no_strips):
        ...
        dist = (extent[2] - extent[0]) / no_strips
        n = 0
        while n < no_strips:
            pt_ll = Point((extent[0] + n * dist), extent[1])
            pt_ur = Point((extent[0] + (n + 1) * dist), extent[3])
    ...
    def query(self, shape):
        ...
        """
        strips_overlaps = self.find_overlapping_strips(shape)
        temp = []
        for strip in strips_overlaps:
